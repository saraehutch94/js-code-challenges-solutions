...
function sayHello() {
  ...
  // Doesn't matter the input, only performs in one step
  ...
  * Coding Challenge Steps:
  1.) Clarify the challenge (read it out loud to yourself,
    re-write the challenge in your own words)
  2.) Talk through your process ("first, I'll declare a function and define a parameter,
  next, I will...")
  3.) Use space purposefully -> have a space for notes, assumptions, tests
  4.) Ask for feedback ("What are your thoughts about my code [so far]?")
  ...
  ********************************************************
  ...
  Write a function called addTwoNumbers that accepts two numeric 
  ...
  // console.log(addTwoNumbers(10, "a"));
  ...
  // As input increases, the amount of steps taken to complete increases
  ...
  // console.log(computeRemainder(8, 0));
  ...
  function reverseUpcaseString(string) {
    startingPoint = string.length - 1;
    reversedString = "";
  
    for (let i = startingPoint; i >= 0; i--) {
      if (string[i].match(/[a-z]/i)) {
        uppercaseStr = string[i].toUpperCase();
        reversedString += uppercaseStr;
      } else {
  ...
  // console.log(reverseUpcaseString("sara hutchinson!!!"));
  ...
  // console.log(removeEnds("hutchinson"));
  ...
  function formatWithPadding(num, string, length) {
    let numAsString = num.toString();
    const lengthToAdd = length - numAsString.length;
    ...
    numAsString = newString + numAsString;
    return numAsString;
  }
  ...
  function isPalindrome(string) {
    ...
    const joinedString = splitString.join("").toLowerCase();
  
    let reversedString = "";
  
    for (let i = joinedString.length - 1; i >= 0; i--) {
      reversedString += joinedString[i];
    }
  
    if (reversedString === joinedString) {
      return true;
    } else {
      return false;
    }
  ...
  // console.log(hammingDistance("!!!!", "****"));
  // console.log(hammingDistance("alc", "a2c"));
  /*-----------------------------------------------------------------
  Challenge: 13-mumble
  ...
  function mumble(string) {
    ...
    for (i = 0; i < string.length; i++) {
      const numOfRepeats = i + 1;
      const repeated = string[i].repeat(numOfRepeats) + "-";
      hyphenatedString += repeated;
    }
    const slicedString = hyphenatedString.slice(0, hyphenatedString.length - 1);
    return slicedString;
  }
  
  // console.log(mumble("Sara"));
  // onsole.log(mumble("Evelyn"));
  // console.log(mumble("Hutchinson"));
  ...
  // console.log(fromPairs(arrayWithArrays));
  // console.log(fromPairs(secondArray));
  ...
  function mergeObjects(object, ...objects) {
    const mergedObject = Object.assign(object, ...objects);
    return mergedObject;
  }
  ...
  function findHighestPriced(arr) {
    ...
    const foundObject = arr.find((obj) => obj.price === highestPrice);
    return foundObject;
  }
  ...
  function mapArray(arr, callback) {
    let newArray = [];
    for (let i = 0; i < arr.length; i++) {
      let callbackResult = callback(arr[i], i);
  ...
  //   for (let i = 0; i < arr.length; i++) {
  //     if ((i = 0)) {
  //       let callbackReturn = callback(accumulator, arr[i], i);
  //       accumulate = callbackReturn;
  //     } else {
  //       let callbackReturn = callback(accumulate, arr[i], i);
  //       accumulate = callbackReturn;
  //     }
  //   }
  //   return accumulate;
...
// console.log(flatten([1, [2, 3]]));
  ...
  function primeFactors(number) {
    ...
    function primeHelper(iteration) {
      if (primeVariable % iteration === 0) {
        primeVariable = primeVariable / iteration;
        primeFactors.push(iteration);
        primeHelper(iteration);
      }
    }
  
    for (let i = 0; i < primeArray.length; i++) {
      primeHelper(primeArray[i]);
    }
  ...
  function intersection(arr1, arr2) {
    ...
    function helper(iteration) {
      for (let i = 0; i < secondArray.length; i++) {
        if (iteration === secondArray[i]) {
          combinedArray.push(iteration);
          secondArray.splice(secondArray[i], 1);
        }
      }
    }
  ...
  function balancedBrackets(string) {
    ...
    while (spliceString.length > 0) {
      let start = (spliceString.length / 2) - 1;
      let end = start + 1;
      let concat = spliceString[start] + spliceString[end];
  
      if (concat === curlies || concat === squares || concat === parens) {
        spliceString.splice(start, 2);
      } else {
        let reverseConcat = spliceString[start - 1] + spliceString[start];
        if (reverseConcat !== curlies && reverseConcat !== squares && reverseConcat !== parens) {
          return false;
        } else {
          let beforeStart = start - 1;
          spliceString.splice(beforeStart, 2);
        }
      }
    }
    return true;
